...
本项目优先使用 [`.cursor/mcp.json`](.cursor/mcp.json) 中声明的 MCP 工具。

| 原则 | 说明 |
|------|------|
| 加载确认 | 自动推进轮开始前应确认 MCP 已在 Cursor Settings → MCP 中加载（修改配置后需重启 Cursor） |
| 浏览器任务 | **必须**优先使用 Playwright MCP（或 Chrome DevTools MCP） |
| UI 实现 | 页面实现必须边实现边浏览器检查（页面、console、network、核心流程） |
| 文件操作 | 必须确认真实文件状态，不得假设写入成功 |
| GitHub | 提交 / push 前必须 `git diff`，避免泄露密钥 |
| MCP 不可用 | 记录原因，按 [docs/agent_skills/mcp_usage_skill.md](docs/agent_skills/mcp_usage_skill.md) 降级继续 |
